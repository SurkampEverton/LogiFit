...
- **Overage documento fiscal** R$ 0,15 / 0,15 / 0,12 / 0,10 / 0,08 / 0,05 por **documento emitido** (NFS-e + NF-e + NFC-e + devolução + transferência + conserto), do Solo ao Enterprise. **Eventos não contam** (cancelamento, CC-e, inutilização). Cota = **1,5× o teto de members** do tier, então quem usa o produto como projetado nunca encosta nela. **Não é repasse de custo** — a conta Focus é do tenant ([ADR 0105](docs/decisions/0105-conta-focus-por-tenant-byo.md)); o overage é guardrail contra abuso + captura de valor pelo ciclo fiscal (fila, retry, reconciliação, storage 5 anos, auditoria, portal do contador). **Pendente de revisão:** com a emissão internalizada ([ADR 0108](docs/decisions/0108-emissao-fiscal-propria-nfe-nfce-nfse.md)) e o Emissor Nacional gratuito, não há mais custo por documento nem para a LogiFit nem para o tenant — a justificativa do overage passa a ser só infraestrutura própria, e o ADR 0066 precisa ser reescrito nesse ponto.
...
- **Cobrança**: Asaas próprio LogiFit + NFS-e automática (emissão própria pelo Emissor Nacional a partir do Sprint 42 — ADR 0108; via Focus até lá).
...
- **Emissão fiscal própria** ([ADR 0108](docs/decisions/0108-emissao-fiscal-propria-nfe-nfce-nfse.md), aceito 2026-07-30 — **supersede os ADRs 0059 e 0076**) — o LogiFit internaliza emissão e motor tributário. Arquitetura de **4 camadas dirigida por dados**: o produto carrega só classificadores (NCM/CEST/origem/perfil fiscal); a tributação vive em `tax_rules` e é resolvida na emissão combinando produto × unidade emissora × destinatário × natureza da operação. Programa em 6 sprints (41-46), caminho crítico **6-8 meses**. **A Focus NFe é rebaixada a fallback** para padrão municipal de NFS-e sem adapter próprio. NT 2013/005 SEFAZ (NFC-e), NT 2011/004 SEFAZ (CC-e), NT 2025.002 (IBS/CBS/IS) passam a ser responsabilidade nossa. Impressão de DANFE/DANFCE/DANFSE em [ADR 0109](docs/decisions/0109-impressao-documentos-fiscais.md).
- **NFS-e pelo Emissor Nacional é o canal único da base LogiFit** — a **Resolução CGSN 189/2026** determina que ME/EPP do Simples prestadoras de serviço emitam NFS-e padrão nacional **exclusivamente pelo Emissor Nacional** (web ou API) a partir de **01/09/2026**; sistemas municipais próprios deixam de ser opção para o Simples. A **LC 214/2025** já obrigava todo município a aderir ao padrão nacional até 01/01/2026. Como a base do LogiFit é Simples, **um adapter cobre o Brasil inteiro** — adapters municipais (ABRASF, IPM, Betha, SIGISS, Carioca, SP) ficam fora do caminho crítico, só para tenant em Lucro Presumido/Real.
- **Nenhuma nota sai sem sombra verde** — enquanto a Focus ainda emitir, o motor próprio calcula em paralelo e um job diário compara com o documento autorizado. Cut-over é por tenant, via flag, só depois de evidência acumulada (regra 47).
...
- [`docs/rules.md`](docs/rules.md) — **47 regras duras** (arquiteturais, multi-empresa, i18n, IA + LGPD, pesquisa global, responsividade, arquitetura IA + erros, escalabilidade banco, segurança em profundidade, assistente IA universal, passaporte cross-tenant, MFA obrigatório, design system "Equilíbrio Vital", mensagens ao usuário, soberania de dependência, emissão fiscal, processo, código). **Em conflito com regras operacionais abaixo, [`docs/rules.md`](docs/rules.md) prevalece como fonte única de verdade** — regras 1-29 abaixo são subset operacional para uso direto pela Claude.
...
> **Numeração canônica:** os números abaixo (1-8, 11, 13-16, 27-47) **são os mesmos** de [`docs/rules.md`](docs/rules.md) — fonte única de verdade. Esta seção é um **digest** das regras mais relevantes ao trabalho diário; lista completa (47 regras + bloco "Convenções de colaboração com Claude" abaixo) está em `rules.md`.
>
...
**28.** **Nunca** ativar feature IA classe SaMD II+ em tenant sem Comitê de IA cadastrado + ata anexada (gate de feature flag). Toda chamada IA clínica grava `ai_audit_log` (input, output, modelo, decisão humana). Classificador de output proibido (`'diagnóstico'`, `'tem [doença]'`, `'prescrever'`) ativo em toda chamada. Ver [ADR 0053](docs/decisions/0053-conformidade-cfm-2454-2026-ia-saude.md).
...
### Emissão fiscal (rules.md 47)

**47.** **Nenhum default tributário é inferido silenciosamente.** Produto sem `fiscal_profile_id`, ou emissão em que nenhuma `tax_rule` casa com o contexto, **aborta antes de qualquer chamada externa** com erro que diz o que falta e onde configurar — nunca emite com CST/CFOP/alíquota chutados. Nota rejeitada custa 5 minutos; nota autorizada e errada é passivo fiscal do cliente, irreversível. Comportamento por CST vive como **flag em `tax_ref_icms_cst`**, nunca `switch` no cálculo. Regra empatada resolve por desempate determinístico (`priority DESC → updated_at DESC → id DESC`). Aritmética em centavos inteiros — float binário proibido na cadeia fiscal. Lint `no-inferred-tax-default`. `tax_ref_*` são catálogo nacional **sem `tenant_id`** (exceção à regra 1, precedente [ADR 0028](docs/decisions/0028-cid-cif-catalogos-globais.md)). Ver [ADR 0108](docs/decisions/0108-emissao-fiscal-propria-nfe-nfce-nfse.md).

Lista completa de regras em [`docs/rules.md`](docs/rules.md) (47 regras duras; índice por bloco no topo do arquivo).
...
- **Email transacional:** **Brevo** (ex-Sendinblue — substitui AWS SES por [ADR 0096](docs/decisions/0096-email-brevo-substitui-aws-ses.md); tier free 300/dia cobre MVP; provider abstrato em `@repo/email` com `BrevoEmailProvider` default + `SmtpEmailProvider` dev local Mailhog + `MockEmailProvider` test)
...
- Módulo fiscal/NF-e (Fase 3 — emissão própria, Sprints 41-46 do ADR 0108)
