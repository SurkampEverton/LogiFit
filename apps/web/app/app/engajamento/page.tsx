...
import { achievements, goals, memberAchievements, rewardGrants } from '@repo/db/schema'
/**
...
export default async function EngajamentoPage() {
  ...
  const [totalAchievements, earnedLast30d, pendingGrants, activeGoals, reachedGoals30d] =
    await Promise.all([
      db
        .select({ n: count() })
        .from(achievements)
      ...
      db
        ...
        .where(
          and(eq(memberAchievements.tenantId, tenantId), gte(memberAchievements.earnedAt, last30d)),
        ),
      db
        .select({ n: count() })
        .from(rewardGrants)
    ...
    <div className="mx-auto max-w-6xl px-6 py-8 space-y-8">
      ...
      >
        ...
        >
          ...
          <div className="text-2xl font-semibold tabular-nums">{totalAchievements[0]?.n ?? 0}</div>
        ...
        >
          ...
          <div
            ...
            style={{
              color: (pendingGrants[0]?.n ?? 0) > 0 ? 'var(--ev-warning, #f59e0b)' : undefined,
            }}
          >
            {pendingGrants[0]?.n ?? 0}
          </div>
      ...
      <section className="rounded-xl border border-dashed border-[color:var(--ev-border)] p-6 text-sm text-[color:var(--ev-text-muted)]">
        <p>
          ...
          <code>member.checked_in</code>, <code>payment.received</code>, <code>goal.reached</code> e
          re-avalia regras de conquista do member via DSL{' '}
          <code>@repo/db/engajamento/evaluator</code>. Brinde físico ganha workflow{' '}
          <code>pending_redeem → redeemed → shipped → delivered</code>.
        </p>
      </section>
    </div>
  )
