...
| **Pesquisa global (Command Palette Ctrl+K) — ADR 0062** | Atalho `Ctrl+K`/`Cmd+K` em qualquer tela abre overlay de busca cross-module respeitando RLS + permission + consent; modificadores `>` ações / `/` rotas / `@` pessoas / `#` tags; full-text (tsvector) + trigram (pg_trgm); audit em clique em dado sensível | todas | 00 (scaffold) + 07 (MVP) + sprints posteriores indexam seus tipos | todo |
