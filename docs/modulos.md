...
| **Planos comerciais LogiFit (ADR 0066 — revisado 2026-04-25)** | **4 tiers principais:** Starter R$ 99 (100 members, **Academia no MVP** — Fisio/Nutri liberam quando módulos entrarem em Fases 2/3, 50 NFS-e inclusas) · Pro R$ 199 (500 members, todas verticais simultâneas + Focus NFe completo + Device Hub + Pipeline Exames + 200 NFS-e) · Business R$ 449 (2.000 members, multi-company, intercompany, adquirência + 1.000 NFS-e) · Enterprise sob consulta (BYOK + SLA + white-label + DPO-as-a-service add-on + 5.000 NFS-e). **Plano Solo R$ 49 / Solo Combo R$ 69** (ADR 0069) para profissional autônomo com UX simplificada. Trial 14d sem cartão (features Pro; dados retidos 30 dias após expiração). **Overage suave R$ 0,50/member** acima do incluído até cap = upgrade sugerido. **Cobrança 1 active member por (paciente, tenant)** — passaporte cross-tenant não duplica (ADR 0077). Cobrança Asaas próprio + NFS-e via Focus NFe. Régua inadimplência D+0 → D+21 read-only → D+45 suspenso → D+135 anonimização | todas | 01a + 04 + 36 | todo |
