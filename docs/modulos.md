...
| **Componentes base responsivos (ADR 0063)** | `<AppLayout>` (sidebar desktop ↔ bottom-nav mobile ↔ drawer tablet) + `<ResponsiveTable>` (table ↔ cards) + `<ResponsiveModal>` (centered ↔ full-screen) + `<ResponsiveForm>` (grid ↔ stack) + `<PortalLayout>` (PWA com safe-area-inset) + tokens touch (`min-h-touch` 44px); 5 breakpoints Tailwind; testes Playwright em 3 viewports obrigatórios | todas | 00 (base) + todos os sprints consomem | todo |
