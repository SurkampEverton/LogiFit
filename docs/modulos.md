...
| **Sistema de mensagens padronizadas (ADR 0089 + regra 45)** | Catálogo fechado de 6 tipos em `packages/ui/components/messages/*`: `<Toast>` (success/info/warning/error/critical via Sonner), `<Banner>` (info/warning/danger persistente), `<AlertDialog>`/`<ConfirmDialog>` (substitui `window.confirm`), `<PromptDialog>` (substitui `window.prompt`), `<FormError>` (inline). Helpers `toast.fromApiError(error)` consomem envelope ADR 0071. `<Toaster nonce={cspNonce}>` CSP-friendly (regra 35). i18n obrigatório (regra 27). a11y por construção (`role="status\|alert\|alertdialog\|dialog"`, focus trap, `aria-describedby`). Lints `no-window-alert` + `no-hardcoded-toast-message` em CI. `<ActionConfirmDialog>` (ADR 0075 IA Camada 3) é wrapper composto sobre `<ConfirmDialog>` deste catálogo. | todas | 00 | todo |
