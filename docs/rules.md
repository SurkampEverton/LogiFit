...
**29.** **Dado de saúde sensível (LGPD art. 11) só trafega com base legal explícita + RIPD vigente.** Todo módulo que processa `health_data` (prontuário, avaliação, exame, mídia clínica, device reading, plano alimentar, prescrição) tem entrada em `ripd_documents` com versão vigente (`ripd_versions`) assinada pelo DPO, revisada no máximo a cada 6 meses. Consent por finalidade (`consent_purposes.lawful_basis`) não pode ser genérico — cada finalidade lista `data_categories[]` e `retention_period` explícitos. CI tem teste que falha se um módulo clínico novo for criado sem registro em `ripd_documents`. **Cadência de maturação obrigatória:** RIPD do módulo deve estar em **v0.5-pre-sprint** (fluxo de dados + 5 camadas de segurança + matriz de risco ≥4 cenários + retenção concreta) **antes** do sprint correspondente passar de "next-up" para "doing"; só v0.1-skeleton bloqueia entrada em desenvolvimento (gate `pnpm compliance:ripd-maturity-check` — Sprint 00). Mesmo gate análogo aplica a `docs/threat-models/` (STRIDE 6-categorias mínimo antes do sprint da feature crítica entrar em "doing"). Direitos do titular (art. 18) atendidos em até **15 dias úteis** (Resolução ANPD nº 2/2024) via portal `/meu/privacidade`. Ver [ADR 0054 §3.1](decisions/0054-lgpd-art11-dados-saude-ripd-versionado.md).
