...
- [~] **Backup off-site (regra 40 — ADR 0091)** — `infra/backup-r2.sh` + `infra/restore-r2.sh` + `docs/runbooks/backup-r2.md` versionados (Faixa 3, 2026-05-11). Script faz `pg_dump` (logifit + glitchtip) → gzip → GPG encrypt → `rclone rcat` pra R2; MinIO via `rclone sync` incremental; Coolify metadata via `tar | gpg`; lokistack configs via tar. Retention 30d via `rclone delete --min-age`. **Ativação pendente:** user precisa (1) criar bucket R2 em `dash.cloudflare.com/r2` → token API com acesso só ao bucket; (2) gerar par GPG `backup@logifit.com.br` em máquina local segura; (3) executar passos 3-4 do runbook no VPS (rclone config + import GPG public + cron entry). Estimativa custo MVP: ~$0.20/mês. Decisão: cron em `/etc/cron.d/` no host (não container — backup precisa acesso ao Docker socket + filesystem `/data/`).
