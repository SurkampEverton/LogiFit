...
- **2026-05-13 — Faixas A+B+C+D entregues (Sprint 11 a 100%).** Treinos core aterrissado: 6 schemas (`exercises` com biblioteca global tenant_id NULL + RLS especial SELECT cross-tenant; `workouts` versionado via parent_workout_id + version int; `workout_items` unique (workout, order) + reps text livre + superset_group; `prescriptions` polimórfico kind+ref_id via FK lógica + check ref_required + ends_after_starts; `workout_sessions` referencia prescription_id pra preservar histórico; `workout_session_items` append-only via ausência de UPDATE/DELETE policy) + 16 RLS policies tenant-scoped + migration drizzle 0017 + 13 unit tests (isolamento per-tenant + biblioteca global visível cross-tenant + INSERT global rejeitado via app-role + check constraints + versionamento + polimorfismo + append-only). `calculateKcalPerSession(items, weightKg, durationMin)` em `@repo/db/treinos` com 11 tests (MET ponderado pelos sets × peso fallback 70kg × duration_hours, clampeado [0, 5000], edge cases). 12 Server Actions wrapped (envelope ADR 0071 + audit): createExercise/listExercises (filtros search/level/muscle, includeGlobal default true), createWorkout (transação com items batch), updateWorkout (versionamento materializado: INSERT nova row version+1 + parent_workout_id + INSERT items), listWorkouts, getWorkout (items expandidos + JOIN exercises pra MET/level/grupos), prescribeWorkout (cria prescription kind=workout, valida member+workout no tenant), listMemberPrescriptions (com JOIN workouts), startSession, recordSessionItem, finishSession (transação preenche calculated_kcal automaticamente: busca workout_items via prescription → calculateKcalPerSession → clamp). UI completa: `/app/biblioteca/exercicios` catálogo combinado global+tenant com filtros + cards com MET/badge GLOBAL; `/app/biblioteca/exercicios/new`; `/app/treinos` lista + `/app/treinos/new` wizard com add 1-a-1 + mover ↑↓ + edição inline séries/reps/carga/descanso/superset; `/app/treinos/[id]` detail read-only com link versão anterior; `/app/members/[id]/treino` ficha completa (prescrições ativas + StartSessionButton via SA + PrescribeWorkoutForm inline + histórico kcal + sessão ativa banner); `/app/members/[id]/treino/sessao/[sid]` execução set-a-set + painel inline RPE+notas pra finalização (regra 45: substitui window.prompt proibido); widget "🏋️ Treinos prescritos" no perfil member entre Metas e slot Copilot. Seed standalone `pnpm db:seed:treinos` popula 20 exercícios globais Compendium 2024 (peito/dorsal/quadriceps/posterior/ombro/braço/core/aeróbico) + 2 workouts (Treino A Superior + Treino B Inferior) com 6 items cada em 8 tenants (16 workouts total). ADR 0023 publicado documentando `prescriptions` polimórfico como base cross-vertical (workout MVP, meal_plan Sprint 29, fisio_protocol Sprint 20) — FK lógica em ref_id, active materializado, versionamento via parent_workout_id, alternativas rejeitadas (tabela-por-vertical / JSONB / STI). 161 testes Vitest verdes em 15 files (eram 137 antes — +13 treinos-rls +11 kcal). Typecheck clean. Preview verificado: biblioteca renderiza 20 cards + wizard renderiza dropdown 20 exercícios. **Pendências menores adiadas Sprint 11+ próximo PR:** cross-prescrição alert ADR 0077 + regra 42 (depende getCrossTenantSummary Sprint 02 + meal_plans Sprint 29 + fisio_protocols Sprint 20); upload vídeo exercício MinIO + URL assinada; job cron diário zera prescriptions.active quando ends_at expira; particionamento workout_sessions/items por trimestre (regra 34 + ADR 0072) Sprint 12+ quando volume justificar; drag-and-drop wizard; E2E Playwright completo (instrutor cria → prescreve → aluno executa → RPE → kcal); feature flag treinos_v1 (PostHog dropado MVP); RIPD `docs/compliance/ripd/v1.0-prescricoes.md` (regra 29 + ADR 0054) — gate de produção clínica; lint `cross-tenant-read-must-log` enforcement nos SAs treinos quando primeira read cross-tenant aterrissar. **Bug pré-existente identificado:** Server Components com `db` global retornam 0 rows porque RLS bloqueia (app.tenant_id não setado nessa conexão; withSessionContext só funciona pra SAs) — mesmo pattern em vendas/members pages; resolução requer mudança infraestrutural fora escopo Sprint 11.
