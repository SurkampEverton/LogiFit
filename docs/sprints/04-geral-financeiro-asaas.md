...
- `tenant_usage_snapshots` — `tenant_id`, `snapshot_date`, `active_member_count int`, `plan_member_limit int`, `fiscal_emissions_count int default 0` (Sprint 36 popula), `ai_calls_count int default 0` (Sprint 06 popula), `plan_tier enum('starter','pro','business','enterprise','solo','solo_combo')`, `member_overage_count int generated`, `member_overage_value_cents int generated`, PK (tenant_id, snapshot_date) PARTITION BY RANGE (snapshot_date) ano. Job diário `compute-tenant-usage-snapshot` recalcula. Widget `/app/settings/tenant/plan` lê.
