...
- [ ] **Função SQL `resolve_ai_class(p_tenant_id uuid, p_feature_key text)`** em `packages/db/functions/resolve-ai-class.sql` — retorna tuple `(samd_class, requires_committee, requires_anvisa_notification, source enum('tenant_override','global'))`. Lógica: SELECT primeiro WHERE `tenant_id = p_tenant_id AND feature_key = p_feature_key AND is_global=false AND active=true`; se vazio, SELECT WHERE `is_global=true AND feature_key = p_feature_key AND active=true`; se ainda vazio, raise exception `feature_not_classified` (segurança: feature IA não classificada não pode rodar).
- [ ] **Gate `withAiClassGate(featureKey, fn)` em `packages/ai/gate.ts`** — chama `resolve_ai_class()`; se `requires_committee=true`, valida que `ai_committee_members` do `tenant_id` solicitante tem ≥1 membro ativo + ata; se falhar bloqueia execução + grava `ai_audit_log` com `gate_blocked=true` + alerta admin.
- [ ] Teste E2E `gate-ai-class.spec.ts`:
  - Cenário 1: feature global classe I (`copilot`) — qualquer tenant chama, gate libera (sem comitê exigido)
  - Cenário 2: feature global classe II (`pipeline_exames`) — tenant sem comitê → bloqueia; tenant com comitê + ata → libera
  - Cenário 3: feature global classe II + tenant cria override classe I (override deliberado documentado em ata) — gate libera com `source='tenant_override'` registrado
  - Cenário 4: feature_key não existe em nenhuma linha → exception `feature_not_classified`
  - Cenário 5: tenant remove todos membros do comitê → próxima chamada classe II+ é bloqueada automaticamente
