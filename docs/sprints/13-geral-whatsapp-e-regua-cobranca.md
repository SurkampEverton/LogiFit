...
- **2026-05-13 — Faixas A+B+C+D core MVP outbound + DSL entregues (Sprint 13 a 100%).** Schemas: 5 tabelas (`message_providers` com `credentials_encrypted jsonb` + `channel`/`provider`/`sandbox`/`active`; `message_templates` com approval_flow `draft→pending→approved` + email pulando direto pra approved + variables auto-extraídas; `reguas` DSL declarativo `trigger jsonb` + `actions jsonb` + `stop_on jsonb` + `guards jsonb`; `regua_executions` state machine `running/completed/stopped_by_rule/stopped_by_consent/failed` + `next_action_at` indexado pra cron tick; `messages_sent` audit-friendly com `variables_resolved jsonb` snapshot + `body_rendered` + `provider_message_id`) + 15 RLS policies tenant-scoped + migration 0019 + 8 unit tests Vitest. DSL régua em `@repo/db/mensagens/dsl.ts`: `ReguaDslSchema` Zod validando 10 eventos canônicos (`invoice.overdue/paid/cancelled`, `member.no_checkin_15d/30d`, `lead.no_response_3d`, `appointment.tomorrow/today`, `workout.session_completed`, `achievement.earned`) + 2 kinds ações (`send_message` com channel/template_slug/delay_days/fallback_channel; `wait` com delay_days) + `StopOnSchema` array eventos + `GuardsSchema` (consent + rate_limit_per_member_24h + hour_window HH:MM); 3 helpers (`nextActionAtFromSteps` calcula próximo trigger somando delays acumulados, `renderTemplate` substitui `{{var.path}}` com edge cases (vars faltantes vazias, espaços tolerados, valores 0 numéricos renderizam), `extractTemplateVariables` lista vars únicas ordenadas, `isWithinHourWindow` GMT-3 SP com janela cruzando meia-noite) + 23 unit tests cobrindo régua canônica válida, eventos não-canônicos rejeitados, action desconhecida rejeitada, actions vazias rejeitada, delay negativo rejeitado, hour_window formato inválido rejeitado, calculation de next_action_at acumulativo, render template múltiplas vars, hour_window cruzando meia-noite. 8 Server Actions wrapped (envelope ADR 0071 + audit_log): createTemplate (extrai variables automaticamente + email skip approval direto pra approved), approveTemplate (atualiza providerTemplateId após aprovação Meta), listTemplates, createRegua (valida ReguaDslSchema antes de gravar trigger/actions/stop_on/guards), activateRegua/pauseRegua, listReguas, sendMessageManual (renderiza body com renderTemplate + resolve member.name/email + grava messages_sent com status='queued' + provider='stub' — envio real Sprint 13b), listMessages com filtros canal/status, listMemberMessages widget perfil. UI completa: `/app/mensagens` hub com 4 cards (templates count + reguas count + histórico count + provider stub placeholder); `/app/mensagens/templates` lista com cards + badges approval (draft/pending/approved/rejected color-coded) + canal icon + variables chips; `/app/mensagens/templates/new` com detecção automática de variáveis enquanto digita corpo (regex `{{...}}` extract + dedup + ordenado) + slug validation + email exige subject; `/app/mensagens/reguas` lista com state badges (ativa/pausada) + trigger event + actions count + runs count; `/app/mensagens/reguas/new` builder visual com trigger picker (6 eventos pre-listados), add actions inline (kind picker send_message/wait + canal + template_slug filtrado por canal + delay_days), stop_on checkboxes (invoice.paid/cancelled), régua nasce inativa por design; `/app/mensagens/historico` com filtros canal/status + cards mostrando channel icon + member name + template name + body rendered + status badge + provider + timestamps delivered/read + failure_reason. Widget perfil member "💬 Mensagens recentes" com até 5 últimas + body rendered + status. Seed standalone `pnpm db:seed:mensagens` popula 5 templates por tenant (cobranca_d1/d3/d7 + reengajamento_15d + boas_vindas — email aprovado direto, WhatsApp em draft) + 1 régua canônica "Cobrança D+1/+3/+7" desativada em 8 tenants = 40 templates + 8 réguas. ADR 0025 atualizado pra **Accepted** documentando schema ready + sub-decisão final Twilio vs Gupshup adiada Sprint 13b. ADR 0026 atualizado pra **Accepted** documentando DSL Zod-validada com 10 eventos + 2 kinds + 3 helpers; evaluator runtime cron tick adiado Sprint 13b. **31 testes Sprint 13 verdes** (8 RLS + 23 DSL). **237 testes total verdes** (era 206). Typecheck `pnpm --filter @app/web typecheck` clean. **Pendências menores adiadas Sprint 13b próximo PR:** envio real WhatsApp/Email via adapter (Twilio/Gupshup/Resend POC + sub-decisão ADR 0025); evaluator runtime cron tick (consome `domain_events` + processa `regua_executions.state='running' AND next_action_at <= now()` + chama provider real); hub inbound multifluxo (ADR 0051 — `whatsapp_inbound_messages` + `whatsapp_conversations` + `tenant_whatsapp_settings` + identity_matcher por persons.phone + intent_router IA + classificador anexos PDF/imagem + `scanUpload` regra 38 + handler registry pluggable consumido por Sprints 15/12/30); webhook callbacks delivery/read; rate limit por (tenant, member) via Redis self-host; opt-out flow via consents `marketing_messages` antes de enviar; UI `/app/mensagens/providers` config credentials_encrypted + sandbox toggle; UI `/app/mensagens/inbound` operador acompanha; RIPD `v1.0-whatsapp.md` (regra 29 + ADR 0054) + DPO sign-off; feature flag `mensagens_v1`; particionamento `messages_sent` por mês (regra 34 + ADR 0072) Sprint 14+; canais de notificação system_alerts via worker consumer. **Bug pré-existente** (Server Components com `db` global retornam 0 rows porque RLS bloqueia) afeta `/app/mensagens/*` lista por tenant — UI funcional, dados apenas via wizard ou seed visíveis quando RLS layer for resolvida.
