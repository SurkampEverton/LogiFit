...
- **Backup off-site grátis** — `pg_dump` semanal cifrado com **GPG** (chave LogiFit em Vercel encrypted env) enviado para **Cloudflare R2** (free tier 10GB storage + 1M class A ops/mês + outbound free para Cloudflare). **Decisão final 2026-04-25: Cloudflare R2** — alinhado a [ADR 0078](0078-hospedagem-duas-fases-mvp-supabase-pos-mvp-oracle.md) (R2 já será storage de mídia em Fase 2; manter mesmo provider reduz operação) e a [ADR 0067](0067-dpo-governanca-compliance-lgpd.md) sub-processors (R2 já listado). Backblaze B2 e GitHub Releases ficam como fallback de **disaster recovery** caso R2 falhe — não são storage primário paralelo. Ativar via Vercel Cron weekly. Retenção 12 meses (rotação automática). Sprint 00 implementa direto contra R2.
