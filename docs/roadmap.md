...
| 9 | [07](sprints/07-geral-dashboard.md) | Dashboard "Equilíbrio Vital" + cross-alert dispatcher | **doing** | 2026-05-13 | — | 70 | Faixas A+B+C entregues (Dashboard `/app` com 7 KPIs + alert_subscribers + dashboard role-aware `/app/dashboard/recepcao` com agenda do dia + check-ins recentes + overdue + Command Palette `<CommandPalette>` Ctrl+K MVP com 20 items canônicos + filtros `>`/`/`); Restante 30% (Faixa D): dashboards diretor/gerente/grupo, ADR 0016 tokens, UI `/app/super-admin/database`, search_index FTS Postgres com Members/Pessoas reais, materialized views | — |
| 10 | [08](sprints/08-academia-controle-acesso.md) | Controle de acesso Academia (QR HMAC + facial opcional + catraca + bloqueio) | **doing** | 2026-05-13 | — | 80 | Faixas A+B+C+D parciais: schemas + RLS + HMAC QR gen/validate com 10 unit tests + 5 Server Actions (registerDevice/revoke/blockMember/unblock/listMemberAccess) + APIs /api/acesso/checkin + heartbeat + UI /app/acesso/checkins feed + ADR 0017 publicado. Restante 20%: ADR 0018 (hardware decision exige POC físico), UI /qr do member rotativo + UI catracas registro, facial recognition (Sprint 09+ RIPD biometria), gate CREF onboarding personal (Sprint 09+) | — |
