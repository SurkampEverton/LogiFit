...
| 8 | [06](sprints/06-geral-copilot-base.md) | **Assistente IA universal base (3 camadas Help/Insight/Action + tool registry distribuído + 7 personas + FAB global + cotas alinhadas a planos — ADR 0064 + 0075)** | **doing** | 2026-05-13 | — | 75 | Faixa A (10 schemas IA) + B (seed 8 models + task_routing 7 tasks + 5 schemas RAG + classifier I/O + redact PII + 7 personas pt/en/es + inferPersona + registry + cache + quotas + 61 unit tests verdes + ADR 0015 publicado) + C (Server Actions sendMessage/proposeAction/confirm/reject/switchPersona + API Routes /api/ai/chat,session,proposals/[id]/confirm,reject + 9 tools whitelist + 3 ai-blocked + support_tickets) + D parcial (AssistantFAB + AssistantSheet + ActionConfirmDialog + páginas /app/assistente /settings/ia /suporte /super-admin/ai-usage + i18n 12 arquivos + RIPD v0.2). Pendente Faixa C/D fechamento: LLM real Vercel AI SDK (stub eco hoje) + RAG seed job (ADRs/sprints/regulações) + BYOK UI write com envelope-crypto + lint `ai-block-respected` + threat model STRIDE + parecer DPO formal | — |
